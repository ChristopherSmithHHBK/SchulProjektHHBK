...
int verbindeDB(void)
{
	...
	//if (!mysql_real_connect(conn, "localhost", "root", "password", "hhbk_projekt", 0, NULL, 0))
	if (!mysql_real_connect(conn, "csmith-design.de", "hhbk_user", "schule1234", "admin_hhbk_prj", 0, NULL, 0))
	{
...
{
	strcpy(cStatement, "select * from benutzer where b_name = '");
...
void holeSQLPruefePasswortStatement(char cStatement[], char cName[], char cPasswort[])
{
	strcpy(cStatement, "select * from benutzer where b_name = '");
	strcat(cStatement, cName);
	strcat(cStatement, "' and b_password = '");
...
{
	strcpy(cStatement, "insert into benutzer (b_name, b_password) values ('");
	strcat(cStatement, cName);
	strcat(cStatement, "','");
	strcat(cStatement, cPasswort);
	strcat(cStatement, "')");
