...
void holeIDsEigeneB(void)
{
   ...
   strcpy(cBefehl,"SELECT b_id FROM benutzer WHERE b_name = '");
...
void eigenebewertung(char ID[])
{
   ...
   strcpy(cBefehl, "SELECT 4ge_bewertung from 4gewinnt where b_id = ");
   strcat(cBefehl, ID);
   ...
   strcpy(cBefehl, "SELECT bri_bewertung from bridges where b_id = ");
   strcat(cBefehl, ID);
      if(mysql_query(conn, cBefehl))
      {
         ausgebenFehlermeldung("SQL Anfrage fehlgeschlagen");
   ...
   strcpy(cBefehl, "SELECT che_bewertung from checkers where b_id = ");
   strcat(cBefehl, ID);
      if(mysql_query(conn, cBefehl))
      {
         ausgebenFehlermeldung("SQL Anfrage fehlgeschlagen");
   ...
   strcpy(cBefehl, "SELECT rev_bewertung from reversi where b_id = ");
   strcat(cBefehl, ID);
      if(mysql_query(conn, cBefehl))
      {
         ausgebenFehlermeldung("SQL Anfrage fehlgeschlagen");
